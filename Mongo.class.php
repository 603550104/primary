...
class Mongo {
    ...
    public function __construct($flag = 'master') {

//        if($flag == 'master'){
//            $this->_mongo = C('Mongo');
//        }else{
//            $this->_mongo = C('Mongo_Slave');
//        }
        $this->_mongo = $this->mongoServerArr[rand(0, 2)];
...
// 		include_once($this->mongo_config);
        ...
        $this->host = trim($this->_mongo['hostname']);
        $this->port = trim($this->_mongo['hostport']);
        $this->user = trim($this->_mongo['username']);
        $this->pass = trim($this->_mongo['password']);

        $this->dbname  = trim($this->_mongo['database']);
        $this->persist = trim($this->_mongo['persist']);
        $this->persist_key = trim($this->_mongo['persist_key']);
     ...
     * @desc sum统计
     * */
    public function group_by_sum_for_log($collection, $searchArr){
        if (empty($collection)) {
            $this->log_error("In order to retreive documents from MongoDB, a collection name must be passed");
            exit;
        }
        ...
        $reduce = "function (obj, prev) {prev.sum = prev.sum + obj.accountValue; prev.count ++;}";
        $count = $this->db->{$collection}->group($keys, $initial, $reduce, $searchArr);
        $this->clear();
        return($count);
    }
     ...
     * @desc 大统计
     * */
    public function group_by_sum_for_total($collection, $searchArr){
        if (empty($collection)) {
            $this->log_error("In order to retreive documents from MongoDB, a collection name must be passed");
            exit;
        }
     ...
     * @desc 大统计
     * */
    public function group_by_sum_for_total_test($collection, $searchArr){
        if (empty($collection)) {
            $this->log_error("In order to retreive documents from MongoDB, a collection name must be passed");
            exit;
        }
        $map = "function(){
                    var key = this.userID;
                    var value = {count : 1, sum : this.accountValue};
                    emit(key, value);
                }";
        $reduce = "function (obj, values) {
                    var ret = {sum : 0, count : 0};
                    values.forEach(function (v) {
                        ret.sum = parseInt(ret.sum + (v.sum));
                        ret.count += v.count;
                    });
                    return ret;
                }";
        $temp = 'tbl_temp_data_' . time() . '_' . mt_rand(10000, 99999);
        $arr = array(
...
//                     '_id' => '$userID',
//                     'count' => array('$sum' => 1),
//                     'sum' => array('$sum' => '$accountValue'),
//                 ),
//             ),
//         );
        
//         $count = $this->db->{$collection}->aggregate($cond);
//         $this->clear();
//         return($count);
    }

    public function group_by_sum_for_total_order($collection, $groupArr, $searchArr, $orderArr, $limit){
        if (empty($collection)) {
            $this->log_error("In order to retreive documents from MongoDB, a collection name must be passed");
            exit;
        }
        $count = $this->db->{$collection}->aggregate($searchArr, $groupArr, $orderArr, $limit);
        $this->clear();
        return($count);
    }

    public function commands($arr){
        if (empty($arr)) {
            $this->log_error("no content");
            exit;
        }
        $count = $this->db->command($arr);
        $this->clear();
        return($count);
    }
     ...
     * @param array $keys 分组的字段
     * @param array $initial 查询字段（分组前变量初始化）
     * @param string $reduce 求分组后数据的方法
     * @param array $searchArr 查询条件
     */
    public function custom_group_search_data($collection, $keys, $initial, $reduce, $searchArr){
        if (empty($collection)) {
            $this->log_error("In order to retreive documents from MongoDB, a collection name must be passed");
            exit;
        }
     ...
     * 管道排序
     */
    public function aggregate($collection, $opts){
        if (empty($collection)) {
            $this->log_error("In order to retreive documents from MongoDB, a collection name must be passed");
            exit;
        }
        return $this->db->{$collection}->aggregate($opts);
    }
}
